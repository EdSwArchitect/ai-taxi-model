...
/**
 * Java record representing a Yellow Taxi trip record from NYC TLC data.
 * Based on the data dictionary from:
 * https://www.nyc.gov/assets/tlc/downloads/pdf/data_dictionary_trip_records_yellow.pdf
 *
 * @param vendorID A code indicating the TPEP provider that provided the record.
 *                 1 = Creative Mobile Technologies, LLC
 *                 2 = Curb Mobility, LLC
 *                 6 = Myle Technologies Inc
 *                 7 = Helix
 * @param tpepPickupDateTime The date and time when the meter was engaged.
 * @param tpepDropoffDateTime The date and time when the meter was disengaged.
 ...
 * @param airportFee For pick up only at LaGuardia and John F. Kennedy Airports.
 * @param cbdCongestionFee Per-trip charge for MTA's Congestion Relief Zone starting Jan. 5, 2025.
 */
public record YellowTaxi(
    ...
    /**
     * Validates the YellowTaxi record.
     *
     * @return true if the record is valid, false otherwise
     */
    public boolean isValid() {
        // Basic validation checks
        if (vendorID == null || (vendorID != 1 && vendorID != 2 && vendorID != 6 && vendorID != 7)) {
            return false;
        }
        if (tpepPickupDateTime == null || tpepDropoffDateTime == null) {
            return false;
        }
        if (tpepDropoffDateTime.isBefore(tpepPickupDateTime)) {
            return false;
        }
    ...
    public Long getTripDurationMinutes() {
        ...
        return java.time.Duration.between(tpepPickupDateTime, tpepDropoffDateTime).toMinutes();
    }
}
