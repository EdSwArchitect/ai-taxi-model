...
/**
 * Reader class for Green Taxi (SHL) Parquet files.
 * Reads Parquet files like green_tripdata_2025_01.parquet, validates the schema,
 * and outputs GreenTaxi objects.
 */
public class GreenReader {
    private static final Logger logger = LogManager.getLogger(GreenReader.class);
    ...
    /**
     * Reads a Parquet file and returns a stream of GreenTaxi objects.
     *
     * @param filePath the path to the Parquet file
     * @return a stream of GreenTaxi objects
     * @throws IOException if there's an error reading the file
     * @throws SchemaValidationException if the schema doesn't match expected fields
     */
    public Stream<GreenTaxi> read(java.nio.file.Path filePath) throws IOException, SchemaValidationException {
        ...
        return Stream.generate(() -> {
            try {
                ...
                return convertToGreenTaxi(record);
            } catch (IOException e) {
    ...
    /**
     * Reads a Parquet file and returns a list of GreenTaxi objects.
     *
     * @param filePath the path to the Parquet file
     * @return a list of GreenTaxi objects
     * @throws IOException if there's an error reading the file
     * @throws SchemaValidationException if the schema doesn't match expected fields
     */
    public List<GreenTaxi> readAll(java.nio.file.Path filePath) throws IOException, SchemaValidationException {
        try (Stream<GreenTaxi> stream = read(filePath)) {
    ...
    /**
     * Converts a GenericRecord from Parquet to a GreenTaxi object.
     *
     * @param record the Parquet record
     * @return a GreenTaxi object
     */
    private GreenTaxi convertToGreenTaxi(GenericRecord record) {
        org.apache.avro.Schema schema = record.getSchema();
        return new GreenTaxi(
            getInteger(record, "VendorID"),
            parseDateTime(record, "lpep_pickup_datetime"),
            parseDateTime(record, "lpep_dropoff_datetime"),
